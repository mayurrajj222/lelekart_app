...
export default function EditProductScreen({ route, navigation }) {
  ...
  const [form, setForm] = useState({
    name: '', brand: '', category: '', subcategory: '', price: '', mrp: '', purchasePrice: '', gstRate: '', warranty: '', returnPolicy: '', description: '', specifications: '', sku: '', stock: '', weight: '', length: '', width: '', height: '', color: '', size: '',
  });
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/api/seller/products/${productId}`, { credentials: 'include' });
        if (!res.ok) throw new Error('Failed to fetch product');
        const data = await res.json();
        setForm({
          name: data.name || '', brand: data.brand || '', category: data.category || '', subcategory: data.subcategory || '', price: data.price?.toString() || '', mrp: data.mrp?.toString() || '', purchasePrice: data.purchasePrice?.toString() || '', gstRate: data.gstRate?.toString() || '', warranty: data.warranty?.toString() || '', returnPolicy: data.returnPolicy || '', description: data.description || '', specifications: data.specifications || '', sku: data.sku || '', stock: data.stock?.toString() || '', weight: data.weight?.toString() || '', length: data.length?.toString() || '', width: data.width?.toString() || '', height: data.height?.toString() || '', color: data.color || '', size: data.size || '',
        });
  ...
  if (loading) {
    return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}><ActivityIndicator size="large" color="#2874f0" /><Text>Loading product...</Text></View>;
  }
