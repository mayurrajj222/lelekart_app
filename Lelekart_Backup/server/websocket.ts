...
// Chat session connections - Maps sessionId to array of WebSocket connections
const chatSessionConnections = new Map<string, WebSocket[]>();

// Admin connections for chat notifications
...
export function setupWebSocketServer(server: http.Server) {
  ...
  wss.on(
    "connection",
    (ws: WebSocket & { isAlive?: boolean }, req: http.IncomingMessage) => {
      ...
      if (isAdmin) {
        // Admin connection for chat notifications
        if (!adminConnections.has(userId)) {
          adminConnections.set(userId, []);
        }
        adminConnections.get(userId)?.push(ws);
        console.log(
          `Admin WebSocket connection associated with userId ${userId}`
        );
        console.log(`Total admin connections: ${adminConnections.size}`);
      } else if (sessionId) {
        // Chat session connection
        if (!chatSessionConnections.has(sessionId)) {
          chatSessionConnections.set(sessionId, []);
        }
        chatSessionConnections.get(sessionId)?.push(ws);
        console.log(
          `Chat WebSocket connection associated with sessionId ${sessionId}`
        );
        console.log(
          `Total chat session connections: ${chatSessionConnections.size}`
        );
      } else {
        // Regular user connection for notifications
        if (!userConnections.has(userId)) {
          userConnections.set(userId, []);
        }
        userConnections.get(userId)?.push(ws);
        console.log(`WebSocket connection associated with userId ${userId}`);
        console.log(`Total user connections: ${userConnections.size}`);
      }
      ...
      ws.on("close", () => {
        ...
        if (isAdmin) {
          ...
          const connections = chatSessionConnections.get(sessionId);
          if (connections) {
            const index = connections.indexOf(ws);
            if (index !== -1) {
              connections.splice(index, 1);
            ...
            if (connections.length === 0) {
              chatSessionConnections.delete(sessionId);
            }
          }
        } else {
          ...
          if (data.type === "ack") {
            ...
            if (data.notificationId) {
              storage
                ...
                .then(() =>
                  console.log(
                    `Notification ${data.notificationId} marked as read`
                  )
                )
                .catch((err) =>
...
export async function sendNotificationToUser(
  ...
  try {
    ...
    if (userSockets && userSockets.length > 0) {
      ...
      console.log(
        `No active WebSocket connections for user ${userId}, notification persisted only`
      );
    }
...
/**
 * Send a chat message to all participants in a chat session
 */
export function sendChatMessage(sessionId: string, message: any) {
  console.log(`Attempting to send chat message to session ${sessionId}`);
  const sessionSockets = chatSessionConnections.get(sessionId);
  console.log(
    `Found ${sessionSockets?.length || 0} sockets for session ${sessionId}`
  );
  ...
  if (sessionSockets && sessionSockets.length > 0) {
    ...
    console.log(`Chat message sent to session ${sessionId}`);
  } else {
    console.log(`No active sockets found for session ${sessionId}`);
  }
}

/**
 * Send a new chat session notification to all admins
 */
export function sendNewChatSessionNotification(session: any) {
  console.log(
    `Attempting to send new chat session notification for session ${session.id}`
  );
  console.log(`Found ${adminConnections.size} admin connections`);
  ...
  const messageJson = JSON.stringify(messageData);
  console.log(`Sending notification:`, messageJson);

  // Send to all admin connections
  adminConnections.forEach((connections, adminId) => {
    console.log(
      `Sending to admin ${adminId}, ${connections.length} connections`
    );
    connections.forEach((socket) => {
      if (socket.readyState === WebSocket.OPEN) {
        ...
        console.log(
          `Admin ${adminId} socket not open, state: ${socket.readyState}`
        );
      }
    });
  });

  console.log("New chat session notification sent to all admins");
}
...
export async function broadcastNotification(
  ...
  try {
    ...
    console.log(
      `Broadcasting notification to ${connectedUserIds.length} connected users`
    );
