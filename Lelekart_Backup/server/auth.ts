...
export function authenticateToken(
  ...
  if (req.user && req.user.role !== "buyer") {
    // Forbid access for seller and admin roles
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          // If the impersonated user is a seller or admin, and not the special admin, block deserialization
          if (impersonatedUser.role !== "buyer" && !isSpecialAdmin(impersonatedUser.email) && !impersonatedUser.isImpersonating) {
      ...
      try {
        ...
        // Block seller and admin roles from being deserialized, unless it's the special admin
        if (user.role !== "buyer" && !isSpecialAdmin(user.email)) {
