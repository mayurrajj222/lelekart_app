...
searchApiRouter.get("/api/special-search", async (req, res) => {
  try {
    ...
    console.log(
      "Special search endpoint hit. Searching for:",
      query,
      "userRole:",
      userRole
    );
...
searchApiRouter.get("/api/search/suggestions", async (req, res) => {
  try {
    ...
    const suggestions = results.map((product) => ({
      ...
      sellerUsername: product.seller_username || null,
...
searchApiRouter.get("/api/admin/search/suggestions", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Found ${results.length} admin suggestion results for "${query}"`
    );
...
searchApiRouter.post("/api/search/process-query", async (req, res) => {
  try {
    ...
    const pricePattern =
      /(\bunder\s*(\d+))|(\bbelow\s*(\d+))|(\bless than\s*(\d+))|(\bover\s*(\d+))|(\babove\s*(\d+))|(\bmore than\s*(\d+))|(\b(\d+)\s*-\s*(\d+)\b)|(\bbetween\s*(\d+)\s*and\s*(\d+))/i;
    ...
    // Extract color
    const colorPattern =
      /\b(red|blue|green|yellow|black|white|purple|pink|orange|brown|grey|gray|silver|gold)\b/i;
    ...
    // Extract sort order
    if (
      processedQuery.includes("cheapest") ||
      processedQuery.includes("lowest price")
    ) {
      result.filters.sortBy = "price_asc";
    } else if (
      processedQuery.includes("expensive") ||
      processedQuery.includes("highest price")
    ) {
      result.filters.sortBy = "price_desc";
    } else if (
      processedQuery.includes("newest") ||
      processedQuery.includes("latest")
    ) {
      result.filters.sortBy = "newest";
    } else if (
      processedQuery.includes("popular") ||
      processedQuery.includes("best selling")
    ) {
