...
export function SimplifiedSearch({ className = "" }: SimplifiedSearchProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (normalizedQuery === lastSubmittedQueryRef.current) {
      console.log(
        "SIMPLIFIED SEARCH - Skipping duplicate search:",
        normalizedQuery
      );
      return;
    }
    ...
    try {
      ...
      history = history.filter(
        (item: string) => item.toLowerCase() !== normalizedQuery.toLowerCase()
      );
      // Add to start
  ...
  return (
    <form
      ...
      className={`relative flex w-full flex-grow max-w-5xl ${className}`}
    >
