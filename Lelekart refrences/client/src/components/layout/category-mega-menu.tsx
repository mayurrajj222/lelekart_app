...
export function CategoryMegaMenu() {
  ...
  const getSubcategoriesForCategory = (categoryId: number): Subcategory[] => {
    ...
    return subcategories.filter(
      (subcategory) =>
        ...
        subcategory.active &&
        (!subcategory.parentId || subcategory.parentId === 0)
    );
  };

  // Get subcategory2 (children) for a given subcategory
  const getSubcategory2ForSubcategory = (
    subcategoryId: number
  ): Subcategory[] => {
    if (!subcategories) return [];
    return subcategories.filter(
      (subcategory) =>
        subcategory.parentId === subcategoryId && subcategory.active
    );
  };
  ...
  const CATEGORY_IMAGE_MAP: Record<string, string> = {
    "Industrial & Scientific":
      "https://www.amazon.in/Industrial-Scientific/b?ie=UTF8&node=5866078031",
    ...
    Beauty:
      "https://beautybybie.com/cdn/shop/files/anti_pigmentation_facial_kit.jpg?v=1749274359",
  ...
  const { data: industrialProducts } = useCategoryProducts(
    "Industrial & Scientific",
    1
  );
  ...
  function getCategoryImage(categoryName: string) {
    switch (categoryName) {
      case "Home":
        return (
          homeProducts?.products?.[0]?.imageUrl ||
          CATEGORY_IMAGE_MAP["Home"] ||
          "/attached_assets/image_1744428587586.png"
        );
      case "Industrial & Scientific":
        return (
          industrialProducts?.products?.[0]?.imageUrl ||
          CATEGORY_IMAGE_MAP["Industrial & Scientific"] ||
          "/attached_assets/image_1744428587586.png"
        );
      case "Fashion":
        return (
          fashionProducts?.products?.[0]?.imageUrl ||
          CATEGORY_IMAGE_MAP["Fashion"] ||
          "/attached_assets/image_1744428587586.png"
        );
      case "Mobiles":
        return (
          mobilesProducts?.products?.[0]?.imageUrl ||
          CATEGORY_IMAGE_MAP["Mobiles"] ||
          "/attached_assets/image_1744428587586.png"
        );
      case "Health & Wellness":
        return (
          healthProducts?.products?.[0]?.imageUrl ||
          CATEGORY_IMAGE_MAP["Health & Wellness"] ||
          "/attached_assets/image_1744428587586.png"
        );
      default:
        return (
  ...
  const categoriesWithoutSubcategories = filteredCategories.filter(
    (category) => category.active && !hasSubcategories(category.id)
  );

  // Categories with subcategories should be dropdown menus
  const categoriesWithSubcategories = filteredCategories.filter(
    (category) => category.active && hasSubcategories(category.id)
  );
                ...
                <DropdownMenu key={category.id}>
                  ...
                  >
                    {getSubcategoriesForCategory(category.id).map(
                      (subcategory) => {
                        const subcategory2List = getSubcategory2ForSubcategory(
                          subcategory.id
                        );
                        return (
                                  ...
                                  {subcategory2List.map((sub2) => (
                                    <div
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        window.location.assign(
                                          `/category/${category.slug}?subcategory=${subcategory.slug}&subcategory2=${sub2.slug}`
                                        );
                                      }}
                                    >
            ...
            {categoriesWithoutSubcategories.map((category) => (
              ...
              >
                ...
                <div className="flex flex-col items-center mb-1">
                  <img
                    src={
                      CATEGORY_IMAGE_MAP[category.name] ||
                      "/attached_assets/image_1744428587586.png"
                    }
                    alt={category.name || "Category"}
