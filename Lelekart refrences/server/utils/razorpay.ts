...
export async function capturePayment(paymentId: string, amount: number): Promise<any> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Payment capture failed: ${JSON.stringify(errorData)}`);
    }
 ...
 * @param paymentId Razorpay payment ID
 * @param amount Amount to refund in paisa (smallest currency unit)
 * @param notes Optional notes for the refund
 */
export async function createRazorpayRefund(paymentId: string, amount: number, notes?: string): Promise<any> {
  if (!isRazorpayConfigured) {
    throw new Error("Razorpay is not configured. Please add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to your secrets.");
  }
  ...
  try {
    ...
    const response = await fetch(`https://api.razorpay.com/v1/payments/${paymentId}/refund`, requestOptions);
    ...
    if (!response.ok) {
      ...
      throw new Error(`Razorpay refund creation failed: ${JSON.stringify(errorData)}`);
    }
...
/**
 * Get refund status for a Razorpay refund
 * @param refundId Razorpay refund ID
 */
export async function getRazorpayRefundStatus(refundId: string): Promise<{status: string, message: string}> {
  if (!isRazorpayConfigured) {
    throw new Error("Razorpay is not configured. Please add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to your secrets.");
  }
  ...
  try {
    ...
    const response = await fetch(`https://api.razorpay.com/v1/refunds/${refundId}`, requestOptions);
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch refund status: ${JSON.stringify(errorData)}`);
    }
    ...
    return {
      status: data.status,
      message: `Refund is ${data.status}`
    };
  } catch (error) {
    console.error('Error getting refund status:', error);
    return {
      status: 'error',
      message: error instanceof Error ? error.message : 'Unknown error checking refund'
    };
  }
}
